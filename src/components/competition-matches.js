...
const CompetitionMatches = React.memo((props) => {
  const { tab } = props;
  ...
  const fetchPagedData = () => {
    ...
    let endpoint =
      "/v1/sports/competition?id=" + competitionid + "&page=" + (page || 1);
    let url = new URL(window.location.href);
    let sub_types = url.searchParams.get("sub_type_id") || "1";
    let tab_info =
      competition_filter === "/upcoming-competition"
        ? "upcoming"
        : competition_filter === "/tomorrow-competition"
        ? "tomorrow"
        : competition_filter === "/highlights-competition"
        ? "highlights"
        : "highlights";
  ...
  let sport_league = Number(
    new URL(window.location).searchParams.get("competition_league")
  );
