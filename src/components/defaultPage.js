...
const DefaultPage = React.memo(() => {
  ...
  const user_slip_validation = useSelector(
    (state) => state.matchesData.user_slip_validation
  );
  const { height, width } = useWindowDimensions();
  let marketName = new URLSearchParams(window.location.search).get(
    "market_name"
  );
          ...
          {bottom_sheet && width < 991 ? (
            ...
            <Right
              betslipValidationData={
                spid ? user_slip_validation_live : user_slip_validation
              }
            ...
            <div ref={bottomSheetRef} className="content">
              ...
              <div className="body d-flex flex-column gap-4">
                {filteredMarkets?.default_markets?.map((market, index) => {
                  ...
                  const pathnameWithLeadingSlash = c_pathname.startsWith("/")
                    ? c_pathname.length == 1
                      ? tab_start
                      : c_pathname
                    : `/${tab_start}`;
                  ...
                  return (
                    <Link
                      ...
                      className={`w-100 markets-default bottom-align ${
                        sub_type === market?.id && "active-market-display"
                      }`}
                      onClick={() => {
                        collapseBottomSheet();
