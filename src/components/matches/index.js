...
const EmptyTextRow = React.memo((props) => {
  ...
  return (
    <button
      ...
      } empty-more-markets-button home-team btn btn-disabled match-detail c-btn justify-content-around d-flex align-items-center ${
        live ? "c-resize" : "width-button-odd"
      }`}
      style={{
        width: "100%",
        height: "40px",
    ...
    >
      {odd_key && (
        <span className=" label btn-disabled " style={{ opacity: "0.3" }}>
          {odd_key}
        </span>
      )}
...
export const marketChoice = () => {
  return [
    ...
    },
    {
      ...
      name: "Over/Under 2.5",
      extra_market_cols: 2,
      extra_markets_display: ["Over", "Under"],
    },
    {
      id: "10",
      name: "Double Chance",
      extra_market_cols: 3,
      extra_markets_display: ["1X", "12", "X2"],
    },
    {
      id: "29",
      name: "Both Teams to Score",
      extra_market_cols: 2,
      extra_markets_display: ["YES", "NO"],
    },
    {
      id: "219",
...
const MoreMarketsHeaderRow = React.memo((props) => {
  const {
    home_team,
    away_team,
    game_id,
        ...
        <>
          <Row className="panel-header primary-bg">
            <h4 className="inline-block">
              <div
                className={
                  "d-flex justify-content-between w-100 align-items-center px-2"
                }
              >
                <div className={"d-flex flex-column w-100"}>
                  <Row className="header-text mb-5 d-flex justify-content-center mt-3">
                    <div
                      className={
                        "d-flex remove-backbutton-on-desktop justify-content-start mx-4 spacing-mobile-web align-items-center"
                      }
                      onClick={() => navigate(-1)}
                    >
                      <FontAwesomeIcon
                  ...
                  <div
                    className={
                      "d-flex w-100  justify-content-between mb-4 align-items-center"
                    }
                  >
                    <div
                      className={
                        "d-flex flex-column team-information_more_markets justify-content-start"
                      }
                    >
                      <FontAwesomeIcon
                        icon={faShield}
                      ...
                      />
                      <span className={"teams-more-markets"}>{home_team}</span>
                    </div>
                    <div
                      className={
                        "team_vs d-flex flex-column team-information_more_markets"
                      }
                    >
                      <span>Vs</span>
                      <span>
                      ...
                      <span>{match_status !== "Ended" && `#${game_id}`}</span>
                    </div>
                    <div
                      className={
                        "d-flex flex-column team-information_more_markets justify-content-end"
                      }
                    >
                      <FontAwesomeIcon
                        icon={faShield}
                        style={{ fontSize: "24px", opacity: "0.7" }}
                      />
                      <span className={"teams-more-markets"}>{away_team}</span>
                    </div>
                  </div>
          ...
          <ButtonGroup
            aria-label="stats button actions"
            className="w-100 d-flex justify-content-start"
          >
            <Button
              className="place-bet-btn w-25 btn link"
              title="scoreboard"
              type="button"
              style={{
...
const SideBets = React.memo((props) => {
        ...
        <>
          ...
          >
            <strong className="more-options-font bold">
              <span className={"stats-mobile more-markets-text text-warning"}>
                +{match.side_bets} More Markets
              </span>
            </strong>
          </Link>
        </>
  ...
  const handleButtonOnClick = useCallback(
    (event) => {
      ...
      const slip = {
        ...
        live: live,
        ucn: clear_rep(
          `${attributes.parent_match_id}${attributes.sub_type_id}${
            attributes.odd_key
          }${marketKey !== undefined ? marketKey : ""}`
        ),
    ...
    (event) => {
      ...
      const slip = {
        ...
        ucn: clear_rep(
          `${jackpot ? attributes.match_id : attributes.parent_match_id}${
            attributes.sub_type_id
          }${attributes.odd_key}${marketKey !== undefined ? marketKey : ""}`
        ),
        market_active: attributes.market_active,
        position: match?.pos || 0,
      ...
      if (cstm === match?.ucn) {
        ...
        if (picked === "picked") {
          ...
          if (
            ...
            betslip =
              jackpot !== true ? addToSlip(slip) : addToJackpotSlip(slip);
...
const MarketRow = React.memo((props) => {
  ...
  const favoriteMarketValue =
    useSelector((state) => state.matchesData.favorites_data) ||
    getFromLocalStorage("favorite_markets") ||
    [];
  ...
  useEffect(() => {
    ...
    const updatedFavoriteValues = combinedFavorites.filter(
      (favorite, index, self) =>
        index === self.findIndex((f) => f.sub_type_id === favorite.sub_type_id)
    );
  ...
  const favoriteMarket = (event, marketId) => {
    ...
    const isFavorite = userFavoriteMarkets.some(
      (favorite) => favorite.sub_type_id === marketId
    );
    ...
    if (isFavorite) {
      ...
      setUserFavoriteMarkets((prevFavorites) =>
        prevFavorites.filter((fav) => fav.sub_type_id !== marketId)
      );
    } else {
  ...
  const setMarketsFavorite = (sub_type_id) => {
    ...
    dispatchRedux(favoriteMarketsData(data))
      ...
      .catch((error) => {
        ...
        setUserFavoriteMarkets((prevFavorites) =>
          prevFavorites.filter((fav) => fav !== sub_type_id)
        );
      });
  };
  ...
  const valuesforPreexpanding = () => {
    const allMarketNames = [
      ...new Set(moreMatches?.data?.odds?.flatMap((item) => item?.sub_type_id)),
    ];
  ...
  return (
    <div className="top-matches match more-markets">
      <Accordion
        preExpanded={valuesforPreexpanding()}
        allowZeroExpanded
        ...
        <AccordionItem className="pb-2" uuid={markets?.sub_type_id}>
          <AccordionItemHeading>
            ...
            >
              <div
                className={
                  "d-flex justify-content-between w-100 more-markets-header-text"
                }
              >
                <span className={"d-flex align-items-center"}>
                  ...
                  <FontAwesomeIcon
                    ...
                    style={{
                      fontSize: "20px",
                      color: userFavoriteMarkets?.some(
                        (favorite) =>
                          favorite.sub_type_id === markets?.sub_type_id
                      )
                        ? "gold"
                        : "white",
                    }}
                    onClick={(event) =>
                      favoriteMarket(event, markets?.sub_type_id)
                    }
...
const ColoredCircle = React.memo(({ color }) => {
...
const getUpdatedMatchFromOdds = (props) => {
  ...
  const ucn = clear_rep(
    newMatch.parent_match_id + "" + newMatch.sub_type_id + odd_key
  );
      ...
      <div className="to-deskview to-block to-tabview  mx-lg-0 px-sm-4 px-md-2 px-lg-0 py-md-4 py-lg-0 mt-2 container-size-match ">
        ...
        >
          ...
          <div className={"to-profile-check separations to-flex-2"}>
            {!jackpot && (
              <>
                {Object.entries(match?.extra_odds || {}).map(
                  ([marketName, odds], index) =>
                    marketName !== "" && (
                      <div
                        key={index}
                      ...
                      >
                        <div className=" flex-row px-1 justify-content-end change-date1 extra-markets-mobile-date">
                          ...
                          <div
                            className={"px-1 wrapping mobile-display-game-id"}
                          >
                            ID: {match?.game_id}
                          </div>
                        </div>
                        ...
                        >
                          {Object.entries(odds || {}).map(
                            ([odd_key, odd_data], index) => (
                              <div
                                key={index}
                                className={
                                  "d-flex flex-column w-100 margin-l-mobile px-sm-1 px-md-1 px-lg-1 "
                                }
                              >
            ...
            <>
              {Object.entries(match?.extra_odds || {}).map(
                ([marketName, odds], index) =>
                  marketName !== "" && (
                    <div
                      className={`c-btn-group  align-self-center to-deskview`}
                      key={index}
                    >
                      {Object.entries(odds || {}).map(
                        ([odd_key, odd_data], index) => {
...
export const MarketList = React.memo((props) => {
  ...
  const matchwithmarkets = useSelector(
    (state) => state.matchesData.more_matches
  );
  ...
  const filterMarketGroups = (group_id) => {
    ...
    if (group_id === "favorite") {
      ...
      filteredMarkets = elements?.filter(
        (market) =>
          Number(market?.group_id) === Number(group_id) || group_id === "all"
      );
    }
    const match = filters?.data?.match;
  ...
  const [market_groups, setMarketGroups] = useState(
    getFromLocalStorage("market_groups")
  );
  useEffect(() => {
    const cache = getFromLocalStorage("market_groups");
  ...
  return (
    <div className="matches full-width" style={{ marginBottom: "0px" }}>
      <div
        className={`web-element ${allMarkets && "bottom-more-markets-mobile "}`}
      ...
      >
        ...
        >
          <Input
            type="text"
            className={
              "form-control h-100  border-0 text-default all-markkets-search "
            }
            style={{
              fontSize: "14px",
              borderRadius: "9px",
            ...
            onInput={(event) =>
              filterMarkets(String(event.target.value).toLowerCase())
            }
            placeholder={"Type to search for market ..."}
          />
        </div>
        <div className="text-white market-groups-container">
          {market_groups?.length > 0 && user && (
            <button
              onClick={() => filterMarketGroups("favorite")}
              className={`market-group-pill text-white badge badge-pill badge-primary bg-transparent p-2 ${
                selectedMarketGroup === "favorite" ? "active-group" : ""
              }`}
            >
              Favorite Markets
            </button>
          )}
          {market_groups?.length > 0 && (
            <button
              onClick={() => filterMarketGroups("all")}
              autoFocus
              className={`market-group-pill text-white badge badge-pill badge-primary bg-transparent p-2 ${
                selectedMarketGroup === "all" ? "active-group" : ""
              }`}
            >
              All Markets
            </button>
          ...
          {market_groups?.map((group, index) => (
            <button
              key={index}
              className={`market-group-pill text-white badge badge-pill badge-primary bg-transparent p-2 ${
                Number(selectedMarketGroup) === Number(group?.id)
                  ? "active-group"
                  : ""
              }`}
        ...
        {marketsToShow.map(([mkt_id, markets], index) => {
          return (
            <MarketRow
              ...
              markets={markets}
              width={
                markets[markets?.market_name].length === 3 ? "33.333%" : "50%"
              }
...
export const JackpotHeader = React.memo((props) => {
  const { jackpot } = props;

  return (
    <Container>
      <Row
        className="top-matches jackpot-resize"
        style={{
