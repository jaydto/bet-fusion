...
const BetHistory = () => {
  ...
  const bet_history_details = useSelector(
    (state) => state.data.bet_history_details
  );
  ...
  const PageBody = () => {
    ...
    const CancelBetMarkup = (props) => {
      ...
      if (
        can_cancel &&
        countdown &&
        bet_id + "cancel_rq" !== getFromLocalStorage("bet_history_status")
      ) {
        return (
          <div className="col d-flex">Cancel will End in: {countdown}</div>
        );
      } else if (
        bet_id + "cancel_rq" === getFromLocalStorage("bet_history_status") &&
        countdown
      ) {
        return <div className="col badge bg-dark rounded-4">CANCEL RQ</div>;
      } else {
        return <div className="col badge bg-dark rounded-4">PENDING</div>;
      }
    };
    ...
    const mybets_data = useSelector(
      (state) => state.matchesData.full_bet_details
    );
    const [mybets, setMybets] = useState(
      state?.filteredHistoryGames || state?.bets_by_date || mybets_data
    );
    ...
    useEffect(() => {
      ...
      state?.bets_by_date,
      getFromLocalStorage("bet_history_filter_category"),
      state?.selected_filter_category,
    ]);
    ...
    const cashoutRequest = (e, bet_id, amount, possible_win) => {
      ...
      setCashoutData(cashout_request_data);
      dispatchRedux(betCashout(cashout_payload));
    };

    const show_cashout_modal = useSelector(
      (state) => state.matchesData.loading_cashout
    );
          ...
          mybets.map((bet, index) => (
            ...
            >
              <div className={"d-flex justify-content-between w-100 px-3"}>
                <div className={"bet-history-items id"}>#{bet?.bet_id}</div>
              ...
              <div className={"d-flex justify-content-between w-100 px-3"}>
                <div className={"bet-history-items date"}>{bet?.created}</div>
                <div className={"bet-history-items status"}>
                  {bet?.can_cancel == 0 ? (
                    <span
                      className={` badge  ${
                        bet?.status_desc == "LOST"
                          ? "bg-dark text-warning"
                          : bet?.status_desc == "WON"
                          ? "bg-success"
                          : bet?.status_desc == "PENDING"
                          ? "bg-dark "
                          : bet?.status_desc == "CASHED OUT"
                          ? "bg-dark "
                          : ""
                      }`}
                      style={{
              ...
              {bet?.status_desc === "PENDING" && bet?.status !== 9 && (
                <div className={"d-flex justify-content-end w-100 px-3"}>
                  <div
                    className={
                      "bet-history-items status d-flex justify-content-end flex-column bet-cashout"
                    }
                  >
                    <span className={"cashout-divider"}></span>
                    {bet?.cashout_status === "1 " ? (
                      <span
                        className={`badge`}
                        style={{
                          ...
                          opacity: "0.5", // Optionally make it visually distinct as disabled
                        }}
                      >
                        Cashout Unavailable
                      </span>
  ...
  const navigateBack = () => {
    if (bet_history_details) {
      ...
      bet_history_details === null ||
      bet_history_details === undefined
    ) {
      window.history.back();
    }
  };
  ...
  const [hideLost, setHideLost] = useState(
    getFromLocalStorage("remove_lost_bets") || false
  );
    ...
    if (
      ...
      if (lost_history) {
        filteredGames = filteredGames?.filter(
          (game) => game?.status_desc !== "LOST"
        );
        dispatch({
          type: "SET",
          key: "filteredHistoryGames",
      ...
      if (state?.selected_filter_category) {
        ...
        if (lostHistory) {
          filteredGamesByDate = filteredGames?.filter(
            (game) => game?.status_desc !== "LOST"
          );
        } else {
        ...
        if (state?.selected_filter_category) {
          ...
          filteredGamesByDate = filteredGamesByDate?.filter((game) => {
            ...
            if (state?.selected_filter_category === "today") {
              ...
              return (
                createdDate >= threeMonthsAgo && createdDate <= currentDate
              );
    ...
    <>
      <>
        <div>
          ...
          <div
            className={"back-navigation original-button top-spacing"}
                    ...
                    <>
                      {bet_history_details == false && (
                        <div className="d-flex w-100 justify-content-between filter-buttons-bethistory px-4">
                          <div
                            className={"filters button-filter text-capitalize"}
                            onClick={showGameHistoryList}
                          >
                            {state?.selected_filter_category || "All"}&nbsp;
                      ...
                      {bet_history_details ? (
                        <BetDetails bet_id={bet_history_details} />
                      ) : (
                        <PageBody />
                      )}
                    </>
