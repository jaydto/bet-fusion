...
let timerInterval;
let timerVar;
const KironPeriods = React.memo((props) => {
  ...
  useEffect(() => {
    ...
    const dataMatches = {
      competition_id:
        ...
        Number(competition_id),
      round_id:
        current_selection_period?.round !== "" ||
        current_selection_period?.round !== undefined
          ? current_selection_period?.round
          : round_id,
      ...
      if (initialTimeLeft <= 10) {
        ...
        if (countdownElement) {
          countdownElement.innerHTML =
            "Match Starts In " + `${initialTimeLeft}`;
        }
      }
  ...
  return (
    <div className={`  container-period `} style={{ background: " #162024" }}>
      {periodsData && (
        <table
          className={"kiron-table"}
          style={{ width: "100%", textAlign: "center", display: "flex" }}
        >
          <tbody className={"d-flex periods"} style={{ overflowX: "auto" }}>
            <tr
              className={
                "d-flex league-row gap-2 justify-content-center align-items-center  kiron-period"
              }
              ref={kironPeriodsRef}
              style={{ flex: "0 0 auto", overflowX: "hidden", height: "50px" }}
            >
              {periodsData?.map((kiron_options, index) => {
                ...
                return (
                  <td
                    key={index}
                    id={`kiron-period-${kiron_options?.round_id}`}
                    className={` d-flex menu-t sport-check w-100 period-card standings-menu ${
                      pathname === kiron_options?.round_id ? " active" : ""
                    }`}
                    style={{ textAlign: "center", lineHeight: "1.5" }}
                  >
                    <div style={{ width: "100%", color: "#000" }}>
                      <div
                        className={` inner-div active d-flex align-items-center kiron-value flex-column justify-content-center link ${
                          currentActivePeriod?.start === startTime &&
                          currentActivePeriod?.round === roundId
                            ? " highlight "
                            : ""
                        } period-height ${
                          isFirst
                        ...
                        {isFirst && !inPlay ? (
                          ...
                          >
                            {Ended
                              ? Ended
                              : `${
                                  play_time > 0 ? "LIVE '" + play_time : "LIVE"
                                }`}
                          </div>
                        ) : (
                          <div style={{ color: "#fff" }} id={`${time}`}>
                            {time}
                          </div>
                        )}
                      </div>
