  ...
  return (
    <button
      ...
      className={`${disabled ? "disabled" : ""} ${
        button_size ? " jackpot-button-placebet " : " "
...
const BetslipSubmitForm = React.memo((props) => {
  const {
    live,
    jackpot,
    totalGames,
  ...
  const [stake, setStake] = useState(
    jackpot
      ...
      : stake_value || getFromLocalStorage("userStake")
  );
  ...
  const updateWinnings = useCallback(() => {
    if (betslip) {
      ...
      let exciseTaxStatus = Number(
        getFromLocalStorage("settings")?.sportsBookLimits?.exciseTaxEnabled ??
          settings?.sportsBookLimits?.exciseTaxEnabled
      );

      let withholdingTaxStatus = Number(
        getFromLocalStorage("settings")?.sportsBookLimits
          ?.withholdingTaxEnabled ??
          settings?.sportsBookLimits?.withholdingTaxEnabled
      );

      // todo check here taxes
      let stake_after_tax = exciseTaxStatus
        ? Float(stake) / Float(112.5 / 100)
        : Float(stake);
      let stake_after_tax_boosted = exciseTaxStatus
        ? (Float(stake) + Float(multiBoostAmount)) / Float(112.5 / 100)
        : Float(stake) + Float(multiBoostAmount);

      let ext = exciseTaxStatus
        ? Float(stake) - Float(stake_after_tax)
        : Float(stake);
      let ext_boosted =
      ...
      if (betslipLength === 1 && !jackpot) {
        if (
          Number(raw_possible_win) >
          (Number(sportBookLimits?.singleBetMaxWin) || 500000)
        ) {
          raw_possible_win = Number(sportBookLimits?.singleBetMaxWin) || 500000;
        }

        if (
          Number(boosted_raw_possible_win) >
          (Number(sportBookLimits?.singleBetMaxWin) || 500000)
        ) {
          boosted_raw_possible_win =
            Number(sportBookLimits?.singleBetMaxWin) || 500000;
        }
      } else if (betslipLength > 1 && !jackpot) {
        if (
          Number(raw_possible_win) >
          (Number(sportBookLimits?.multiBetMaxWin) || 500000)
        ) {
      ...
      let wint_boosted = withholdingTaxStatus
        ? taxable_amount_boosted * 0.2
        : 0;

      let nw = withholdingTaxStatus
        ? raw_possible_win - wint
        : raw_possible_win;
      let nw_boosted = withholdingTaxStatus
        ? boosted_raw_possible_win - wint_boosted
        : boosted_raw_possible_win;
      ...
      dispatchRedux(
        setMatchBetslipOptions("betslip_options", {
          ...betslip_options,
          ...{ netWin: Float(nw, 2), netWinBoosted: Float(nw_boosted, 2) },
        })
      );
  ...
  const value_for_odds_change =
    getFromLocalStorage("accept_all_odds_change") === undefined
      ? true
      : getFromLocalStorage("accept_all_odds_change") === null
      ? true
      : getFromLocalStorage("accept_all_odds_change");
  const initialValues = {
    bet_amount:
  ...
  const calculateMultiBetBoostAmountCallback = useCallback(() => {
    ...
    if (totalGames < giftMinGames) {
      ...
      dispatchRedux(
        setMatchBetslipOptions("betslip_options", {
          ...betslip_options,
          ...{ hasBoost: false, alert_slip_color: "not_qualified" },
        })
      );
    }
      ...
      dispatchRedux(
        setMatchBetslipOptions("betslip_options", {
          ...
          ...{
            ...
            hasBoost: false,
            alert_slip_color: "not_qualified",
        ...
        dispatchRedux(
          setMatchBetslipOptions("betslip_options", {
            ...
            ...{
              ...
              remaining_games: 0,
              alert_slip_color: "valid",
              multiboostmessage: "You Have Qualified for a Nare Boost  ",
            },
          })
        );

        setMultiBoostMessage("You  Have Qualified for a Nare Boost ");
      }
    } else {
      setMultiBoostAmount(0);
  ...
  const show_share_modal = useSelector(
    (state) => state.matchesData.show_share_modal
  );
  const share_bet = useSelector((state) => state.matchesData.share_bet);
  const loadingShare = useSelector(
    (state) => state.matchesData.loading_bet_history
  );

  const [showShareModal, setShowShareModal] = useState(false);
  const [betSharePayload, setBetSharePayload] = useState({});
  const show_deposit_modal = useSelector(
    (state) => state.betting.insufficient_balance
  );
      ...
      {(props) => {
        ...
        const onFieldChanged = (ev) => {
          ...
          if (field === "bet_amount") {
            ...
            let message = {
              status: 401,
              message:
                "You have reached the maximum allowable stake for this bet",
              token: "",
            };
            let minStakeMessage = {
              message: `Minimum amount is ${sportBookLimits?.singleBetMinStake} KSH`,
            };
                          ...
                          hasMultiBetBoost
                            ...
                            : Number(
                                getFromLocalStorage("settings")
                                  ?.sportsBookLimits?.withholdingTaxEnabled ??
                                  settings?.sportsBookLimits
                                    ?.withholdingTaxEnabled
                              )
                            ? withholdingTax
                            : 0
        ...
        return (
          <FormikForm name="betslip-submit-form">
            ...
            <div>
              {!jackpot && multiBoostMessage && (
                <div
                  className={` slip-message-alert`}
                ...
                >
                  ...
                  <td
                    colSpan={2}
                    className={" bet-align-right betslip-alert-close"}
                  >
                    <input
                      type="submit"
                      value="X"
            ...
            {totalGames > 0 && (
              <div className="bet-table w-100 box-shadow-table-submit-form ">
                <div className={"slip-body"}>
                  ...
                  {!jackpot && (
                    <div className="hide-on-affix mt-2 d-flex justify-content-between p-lg-2 p-md-2 py-sm-0 ">
                      <div
                        className={
                          "bet-align-left d-flex align-items-center bet-select-values"
                        }
                      >
                        Total Odds
                      </div>
                  ...
                  <div className="bet-win-tr hide-on-affix d-flex justify-content-between ">
                    <div
                      className={
                        " bet-align-left d-flex align-items-center  show-container bet-select-values p-lg-2 p-md-2 py-sm-0"
                      }
                    >
                      ...
                      <span onClick={() => showUserInfo()} className={"bold"}>
                        <FontAwesomeIcon
                          icon={faInfoCircle}
                          className={"show-values-betslip"}
                        />
                        {showInfo && <UserInfoContainer />}
                      </span>
                    </div>
                    <div
                      className={"bet-align-right d-flex align-items-center"}
                    >
                      KES.{" "}
                      <strong id="net-amount">
                        {formatNumber(
                          jackpot
                  ...
                  {user && !jackpot && (
                    <div className="hide-on-affix d-flex justify-content-between p-lg-2 p-md-2 py-sm-0">
                      <div
                        className={
                          "bet-align-left nare-boost-color d-flex align-items-center"
                        }
                      >
                        Nare Boost &nbsp;
                        <FontAwesomeIcon
                          icon={faBolt}
                          className={"boost-betslip"}
                        />
                      </div>
                  ...
                  <div
                    className={
                      "d-flex align-items-center container-styling-input-placebet mt-2 p-lg-2 p-md-2 py-sm-0 "
                    }
                  >
                    <div className={"bg-input-placebet"}>Amount (KES)</div>
                    <div className={"w-100"}>
                      <div id="betting">
                        {jackpot ? (
                  ...
                  <div
                    className={
                      "w-100 justify-content-end p-1 d-flex min-skake-container"
                    }
                  >
                    {minStake?.message && (
                      <span className={"min_stake_alert"}>
                        {minStake?.message}
                      </span>
                    )}
                  </div>
                  ...
                  <div className={"d-flex gap-2 align-items-center"}>
                    <div
                      className="bet-win-tr hide-on-affix col-4"
                    ...
                    <div id="odd-change-text" className={"col-8"}>
                      <div
                        className={
                          "d-flex bet-select-values w-100  p-lg-2 p-md-2 py-sm-0"
                        }
                        style={{ whiteSpace: "nowrap" }}
                        ref={scrollToRef}
                      >
